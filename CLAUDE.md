...
- `risk/manager.py` — `RiskManager.size(signal) -> SizedOrder`. Three sizing strategies: `percent` (sized as a % of available balance, where balance is already net of in-flight orders thanks to BotManager subtraction), `fixed` (constant USD), `mirror` (`source_notional × mirror_scale`, with `min_trade_usd` floor — sub-floor signals are skipped via `RiskRejection`, never boosted). All strategies then go through per-trade %, per-market, and daily-loss caps. Raises `RiskRejection` on reject.
